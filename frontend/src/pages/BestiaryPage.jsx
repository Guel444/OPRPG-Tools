...
import { ElementBadge, VDBadge, Input, Select, EmptyState, Spinner, Card } from '../components/ui';
...
export default function BestiaryPage() {
  ...
  const { data: creatures, loading } = useCatalog('creatures', params);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6 flex flex-col gap-3">
        ...
        {showFilters && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 rounded border border-stone-800 bg-stone-900/40">
      ...
      {loading ? (
        ...
        <>
          <div className="text-xs text-stone-600 mb-3" style={{ fontFamily: 'var(--font-display)', letterSpacing: '0.1em' }}>
            {creatures.length} AMEAÇA{creatures.length !== 1 ? 'S' : ''} ENCONTRADA{creatures.length !== 1 ? 'S' : ''}
          </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {creature.is_secret && (
                          <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'rgba(192,57,43,0.1)', border: '1px solid rgba(192,57,43,0.25)', color: '#e57373', fontFamily: 'var(--font-display)', fontSize: '8px', letterSpacing: '0.1em' }}>
                            ARQUIVO SECRETO
                          </span>
                        )}
                      </div>
